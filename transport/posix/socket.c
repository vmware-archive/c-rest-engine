    ...
    )
{
    ...
    BAIL_ON_VMREST_ERROR(dwError);
     ...
     VMREST_LOG_DEBUG(pRESTHandle,"BEFORE: nBufData %u, nProcessed %u",pSocket->nBufData, pSocket->nProcessed);
    bLocked = TRUE;
    if (pSocket->pszBuffer)
    {
         VMREST_LOG_DEBUG(pRESTHandle,"%s","Copying prev buf data");
    ...
    do
    {
        ...
        VMREST_LOG_DEBUG(pRESTHandle,"Socket read nREad %d, EC %u,  errno %u", nRead,  errorCode, errno);

        if ((nRead > 0) && (nRead <= MAX_DATA_BUFFER_LEN))
        {
            VMREST_LOG_DEBUG(pRESTHandle,"BEFORE nPrevBuf %u", nPrevBuf);
            nPrevBuf += nRead;
            dwError = VmRESTReallocateMemory(
                  (void*)pszBufPrev,
                  (void **)&pszBufPrev,
            ...
            BAIL_ON_VMREST_ERROR(dwError);
            memset((pszBufPrev + nPrevBuf), '\0', MAX_DATA_BUFFER_LEN);
            VMREST_LOG_DEBUG(pRESTHandle,"After nPrevBuf %u", nPrevBuf);
        }
    }while(nRead > 0);
...
cleanup:
    ...
    if (bLocked)
    {
        VmRESTUnlockMutex(pSocket->pMutex);
         ...
         VMREST_LOG_DEBUG(pRESTHandle,"After: nProcessed %u, nBufDatai %u, nBufLen %u, nPrevBuf %u, dwError %u", pSocket->nProcessed, pSocket->nBufData, *nBufLen, nPrevBuf, dwError);
    return dwError;
    ...
    )
{
    uint32_t                         dwError = REST_ENGINE_SUCCESS;
    int                              timerFd = INVALID;
    int                              mSec = 60000;  //Timer fires after 10000 milliSecond
