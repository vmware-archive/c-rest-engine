    ...
    )
{
    ...
    *nProcessed = 0;
    ...
    VMREST_LOG_DEBUG(pRESTHandle,"nBytes %u", nBytes);
    ...
    if (pszEndNewLine != NULL)
    {
        ...
        if (nLineLen < MAX_REQ_LIN_LEN)
        {
            ...
            else
            {
                 ...
                 {
                     VMREST_LOG_DEBUG(pRESTHandle,"%s", "No Space");
                 }
                 else if ((pszFirstSpace - pszStartNewLine) >= MAX_METHOD_LEN)
                 {
                      VMREST_LOG_DEBUG(pRESTHandle,"PointerDiff %u", (pszFirstSpace - pszStartNewLine));
                 }
                 else
                 {
                     VMREST_LOG_DEBUG(pRESTHandle,"%s", "I dont know");
                 }
    ...
    )
{
    ...
    *nProcessed = 0;
    ...
    {
        nLineLen = pszEndNewLine - pszStartNewLine;
        VMREST_LOG_DEBUG(pRESTHandle,"nLineLen %u, nBytes %u", nLineLen, nBytes);
...
cleanup:
    VMREST_LOG_DEBUG(pRESTHandle,"Bytes Processed this read %u", *nProcessed);
    return dwError;
    ...
    )
{
    ...
    *nProcessed = 0;
    ...
    VMREST_LOG_DEBUG(pRESTHandle,"nCopyByrtes %u, Data Remaining %u", nCopyBytes, pRequest->dataRemaining);
    if (nCopyBytes > 0)
    {
    ...
    while (!((nProcessed == 0) && (currState == prevState)))
    {
        ...
        switch(currState)
        {
            case PROCESS_REQUEST_LINE:
                 VMREST_LOG_DEBUG(pRESTHandle,"Going to Process Req Line, nBytes %u, nTotalProcessed %u", nBytes, nTotalProcessed);
                 ...
                 BAIL_ON_VMREST_ERROR(dwError);
                 VMREST_LOG_DEBUG(pRESTHandle," nProcessed %u", nProcessed);
                 break;
...
cleanup:
    VMREST_LOG_DEBUG(pRESTHandle," Totalprocesse %u", nTotalProcessed);
