    ...
    )
{
    ...
    *nProcessed = 0;
    ...
    {
        nLineLen = pszEndNewLine - pszStartNewLine;
        VMREST_LOG_DEBUG(pRESTHandle,"Start processing header line, nLineLen %u, startnewline ptr %p", nLineLen, pszStartNewLine);
        if (nLineLen == 0)
        {
        ...
        if (pszColonSeparator)
        {
            ...
            {
                VMREST_LOG_ERROR(pRESTHandle,"Header name empty or too large, AttLen %u, nLineLen %u, startnewline %p", nAttrLen, nLineLen, pszStartNewLine);
                temp = (char*)calloc((nAttrLen+1),1);
                memcpy(temp, pszStartNewLine, nAttrLen);
                VMREST_LOG_ERROR(pRESTHandle, "Header line is %s", temp);
                dwError = BAD_REQUEST;
                BAIL_ON_VMREST_ERROR(dwError);
            }
    ...
    else     /**** pszEndNewLine = NULL  ****/
    {
        ...
        temp2 = (char*)calloc((nBytes+1), 1);
        memcpy(temp2, pszStartNewLine, (nBytes));
        VMREST_LOG_ERROR(pRESTHandle, "Incomplete line is %s", temp2);
    }
